...
public class DatabaseController {
    ...
    public ResponseEntity<?> getDBById(@PathVariable String name) {
        log.info("Find db");
        HashMap<String, Resource> databaseResources = new HashMap();
        databaseResources.put("tables", new Resource("/db/" + name + "/table", "get"));
        return ResponseEntity
                .status(OK)
                .body(Hateoas.formatResponse(Serializer.Serialize(databaseManager.getDatabases().get(name)), databaseResources));
    }
    ...
    public ResponseEntity<?> createNewDB(@Valid @RequestBody String databaseRequest) {
        ...
        HashMap<String, Resource> databaseResources = new HashMap();
        databaseResources.put("tables", new Resource("/db/" + request.getName() + "/table", "get"));
        databaseResources.put("db", new Resource("/db", "get"));
        return ResponseEntity
                .status(CREATED)
    ...
    public ResponseEntity<?> update(@PathVariable String name, @RequestBody String databaseRequest) {
        ...
        return ResponseEntity
                .status(ACCEPTED)
                .body(Hateoas.formatResponse(Serializer.Serialize(db.update(request)), databaseResources));
    }
    ...
    public ResponseEntity<String> deleteDBById(@PathVariable String name) {
        ...
        databaseResources.put("db", new Resource("/db", "get"));
        return ResponseEntity
                .ok().body(Hateoas.formatResponse("Delete successful", databaseResources));
    }
}
